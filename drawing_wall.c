...
void	renderer_center(t_form *form, t_player *player, double start, double end, t_doom *doom, int bul)
{
...
{
	pif_a = pifagor(form->vertex.x2 - player->x, form->vertex.y2 - player->y);
...
void	renderer_left(t_form *form, t_player *player, double start, double end, t_doom *doom, int bul)
{
	printf("LEFT DRAWING!!!!!!!!!!!!!!\n");
	double pif_a;
	double pif_b;
...
void	renderer_right(t_form *form, t_player *player, double start, double end, t_doom *doom, int bul)
{
...
printf("%f\n", bsp->node_form->dist);
	...
	printf("FFFIIIRRRSSSTTTT::::::::angle_a:%f=========angle_b:%f\n", angle_a, angle_b);
	...
	else if (is_right_a >= 0 && is_right_b >= 0)
	{
		...
		printf("angle_a:%f\n, angle_b:%f\n", angle_a, angle_b);/////////////////////////////////////////////последний аргумент везде держится на соплях!!!
		renderer_right(bsp->node_form, player, ft_min_d(angle_a, angle_b), ft_max_d(angle_a, angle_b), doom, angle_a < angle_b);
	}//																										 ^^^^^^^^^^^^^^^^^
	else if (is_right_a <= 0 && is_right_b <= 0)
	{
		angle_a = angle_a * player->angle_coef;
		angle_b = angle_b * player->angle_coef;
		printf("angle_a:%f\n, angle_b:%f\n", angle_a, angle_b);
		renderer_left(bsp->node_form, player, ft_max_d(angle_a, angle_b), ft_min_d(angle_a, angle_b), doom, angle_b < angle_a);
	}
	else
	{
		angle_a = angle_a * player->angle_coef;
		angle_b = angle_b * player->angle_coef;
		printf("angle_a:%f\n, angle_b:%f\n", angle_a, angle_b);
		// renderer_center(bsp->node_form, player, angle_a, angle_b, doom, angle_a > angle_b);
		if (is_right_a < 0)
			renderer_center(bsp->node_form, player, angle_a, angle_b, doom, 1);
		else
			renderer_center(bsp->node_form, player, angle_b, angle_a, doom, 0);
	}
}
